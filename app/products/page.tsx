...
function resolveBackendFilters(category?: string, sub?: string): BackendFilters {
  const normalizedCategory = category ? normalize(category) : "";
  const normalizedSub = sub ? normalize(sub) : "";

  let categoryName: string | undefined = normalizedCategory === "pokemon-tcg"
    ? "Pokemon TCG"
    : undefined;
...
function buildProductsUrl(sort: string, page: number, category?: string, sub?: string, q?: string) {
...
function sortProducts(products: Product[], sort: string): Product[] {
  ...
  if (sort === "price-desc" || sort === "default") return arr.sort((a, b) => (b.price ?? 0) - (a.price ?? 0));
  return arr;
}
...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  const sortHref = (nextSort: string) => buildProductsUrl(nextSort, 1, category, sub, q);
  const pageHref = (nextPage: number) => buildProductsUrl(sort, nextPage, category, sub, q);
        ...
        <div className="flex-1">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              <span className="text-gray-500">
                Trang {safePage}/{totalPages} - {filteredProducts.length} sản phẩm
              </span>
          ...
          {products.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-24 text-center text-gray-500">
              ...
              <Link href="/products" className="text-sm text-(--brand-red) hover:underline">
                Xem tất cả sản phẩm
              </Link>
            </div>
          ) : (
