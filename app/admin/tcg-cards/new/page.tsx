...
export default function NewTcgCardPage() {
  ...
  const [payload, setPayload] = useState<AdminTcgCardUpsert>({ cardId: "", setId: "", name: "", cardNumber: "", rarity: null, imageSmall: null, imageLarge: null });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm"><div className="mb-1 font-medium">Set ID</div><select required value={payload.setId} onChange={(e) => setPayload({ ...payload, setId: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2"><option value="">Select set</option>{setOptions.map((set) => (<option key={set.setId} value={set.setId}>{set.setId} - {set.name}</option>))}</select></label>
      ...
      <div className="flex gap-2"><button disabled={loading} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">{loading ? "Saving..." : "Create"}</button><button type="button" onClick={() => router.push("/admin/tcg-cards")} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold">Cancel</button></div>
    </form>
  );
}
