  ...
  return (
    <EntityTable
      ...
      columns={[
        ...
        { header: "Code", render: (row) => <span className="font-semibold">{row.code}</span> },
        { header: "Name", render: (row) => row.name },
        { header: "Discount", render: (row) => `${row.discountType} - ${formatCurrency(row.discountValue)}` },
        ...
        { header: "Status", render: (row) => boolBadge(row.isActive) },
        {
          header: "Action",
          render: (row) => (
            <Link href={`/admin/vouchers/edit/${row.id}`} className="text-indigo-600 underline">
              Edit
            </Link>
          ),
        },
